...
[![Build Status](https://travis-ci.org/HectorTa1989/DHCP-server-client-NodeJS.svg?branch=master)](https://travis-ci.org/HectorTa1989/DHCP-server-client-NodeJS)
[![MIT license](http://img.shields.io/badge/license-MIT-brightgreen.svg)](http://opensource.org/licenses/MIT)

DHCP-server-client-NodeJS is a RFC compliant DHCP client and server implementation on top of node.js.
...
When installed globally, DHCP-server-client-NodeJS provides two executables, a client `dhcp` and a server `dhcpd`. The client simply retrieves network configuration from a DHCP server and prints the configuration after a complete handshake. All additional (defined in `lib/options.js`) fields can be specified as list of arguments:
...
```bash
# Build the image
docker build -t HectorTa1989/DHCP-server-client-NodeJS:0 .
# Start the server
docker run -d --name dhcpd HectorTa1989/DHCP-server-client-NodeJS:0
...
docker build -t HectorTa1989/DHCP-server-client-NodeJS:0 .
# Start the server
docker run --net=host -d --name dhcpd HectorTa1989/DHCP-server-client-NodeJS:0
```
...
docker build -t HectorTa1989/DHCP-server-client-NodeJS:0 .
# Start the servers
docker run -p the.host.ip:67:67/udp -d --name dhcpd1 HectorTa1989/DHCP-server-client-NodeJS:0
docker run -p other.host.ip:67:67/udp -d --name dhcpd2 HectorTa1989/DHCP-server-client-NodeJS:0
```

Installation
===

Installing DHCP-server-client-NodeJS is as easy as cloning this repo or use npmjs:
...
DHCP-server-client-NodeJS does not set timers already on the client to periodically send RENEW or REBIND messages. If you need this feature, please file a bug ticket.
